...
export default function PricingPage() {
  const handleCheckout = async () => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Erro de conexão com o servidor.");
    }
  };
      ...
      <main className="flex-1 w-full flex flex-col items-center py-20 px-8">
        <div className="max-w-4xl w-full text-center mb-20">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <span className="text-[#84A59D] font-black tracking-[0.3em] uppercase text-[10px] mb-4 block">
              Investimento em você
            </span>
            <h1 className="text-5xl font-black text-[#1F2937] tracking-tight mb-6">
              A constância não precisa ser cara, mas precisa ser protegida.
            </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl w-full px-4">
          ...
          >
            ...
            <Link
              href="/dashboard"
              className="w-full py-4 rounded-2xl border-2 border-[#E5E7EB] text-[#64748B] font-bold text-center hover:bg-[#F9FAFB] transition-all"
            >
              Continuar Grátis
            </Link>
          </motion.div>
        ...
        <p className="mt-12 text-[#9CA3AF] text-sm font-medium flex items-center gap-2">
          <Shield className="h-4 w-4" /> Pagamento seguro via Pagar.me. Cancele quando quiser.
        </p>
      </main>
    </div>
  );
